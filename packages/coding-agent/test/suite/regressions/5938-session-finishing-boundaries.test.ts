...
import type { AgentFamilyRosterResult, AgentSessionMessageController } from "../../../src/core/agent-messages.js";
...
describe("session finishing boundaries", () => {
	...
	it("keeps each messaging-controller read live and preserves receiver and promise identity", async () => {
		const failure = new Error("roster failed");
		const pending = Promise.reject<AgentFamilyRosterResult>(failure);
		void pending.catch(() => {});
		const unused = () => {
			throw new Error("unused messaging operation");
		};
		const first: AgentSessionMessageController = { listAgents: unused, sendAgentMessage: unused, roster: unused };
		...
		const result = harness.session.handleAgentMessageHostRequest("agent_message.list_agents");
		expect(reads).toBe(3);
		expect(result).toBe(pending);
		await expect(result).rejects.toBe(failure);
	});
