...
describe("session input and turn ownership", () => {
	...
	it("preserves public clear wrappers and advances cancellation identity before dispatch", async () => {
		const harness = await createHarness();
		harnesses.push(harness);
		const session = harness.session;
		...
		const result = { steering: ["wrapped"], followUp: [] };
		const clear = vi.spyOn(session, "clearQueuedUserMessagesMatching").mockImplementation(function (
			this: typeof session,
		) {
			expect(this).toBe(session);
		...
		expect(session.clearQueuedAgentMessages()).toBe(result);
		...
		expect(() => session.clearQueuedAgentMessages()).toThrow("clear unavailable");
		expect(session.getFollowUpMessages()).toEqual(["protected queued message"]);
	});
