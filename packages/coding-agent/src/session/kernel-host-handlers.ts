...
type MessageResult = AgentSessionMessageListResult | AgentSessionMessageReceipt | AgentFamilyRosterResult;
...
export interface SessionKernelOperations {
	...
	handleMessage(type: string, payload?: Record<string, unknown>): MessageResult | Promise<MessageResult>;
...
export function createSessionKernelHostHandlers(host: SessionKernelOperations): HostRequestHandlers {
	...
	if (host.getAgentMessageController() && visibleKernelSkillNames.has(AGENT_MESSAGE_SKILL_NAME)) {
		Object.assign(
			handlers,
			createAgentMessageHostHandlers({
				roster: async () => (await host.handleMessage("agent_message.list_agents")) as AgentFamilyRosterResult,
				...
				sendAgentMessage: async (input) => {
					...
					if (host.getDepth() > 0) {
						...
						const controller = host.getAgentMessageController();
						if (input.receiverRole === undefined && controller?.roster) {
							try {
